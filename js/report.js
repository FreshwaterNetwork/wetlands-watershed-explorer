    ...
    populateWetlandList: function (t, evt) {
      if (evt.features.length > 0) {
        if (t.obj.wetlandListArray.length < 5) {
          ...
          t.wetlandReportObj[evt.features[0].attributes.OBJECTID] = {
            WETLAND_TYPE: wetType,
            "FA Rank": fa,
            "SS Rank": ss,
            "PR Rank": pr,
            "NR Rank": nr,
          ...
          $.each(t.obj.wetlandListArray, function (i, el) {
            $("#" + t.id + "wetlandSelectedList").append(
              '<div class="wfa-wetlandSelected">Wetland Id: <span>' +
                el +
                " </span><a>Remove</a></div>"
            );
          });
      ...
      $("#" + t.id + "wetlandSelectedList a").on("click", function (c) {
        ...
        let index = t.obj.wetlandListArray.indexOf(
          parseInt($(c.currentTarget).parent().find("span").html())
        );
        ...
        $("#" + t.id + "wetlandSelCounter")
          .find("span")
          .first()
          .html(t.obj.wetlandListArray.length);
      });
      // update the HTML in the wetland selected counter
      $("#" + t.id + "wetlandSelCounter")
        .find("span")
